...
export const bookmarkVideo = async (userId, videoId) => {
  try {
    ...
    console.log("Bookmarks by user",user.bookmarks);
    ...
    console.log("Bookmark", updatedBookmark);
    ...
    console.log("Video bookmarked successfully", updatedBookmark);
  } catch (error) {
    throw new Error(error);
  }
};
...
export const removeBookmark = async (userId, videoId) => {
  try {
    ...
    // Debugging: Log the existing bookmarks
    console.log("Existing bookmarks:", user.bookmarks);

    // Ensure we are filtering correctly
    const updatedBookmarks = user.bookmarks.filter((bookmark) => {
      // Check if bookmark is an object or a string
